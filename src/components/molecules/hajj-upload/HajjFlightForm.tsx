...
import {
  ...
  toggleFlightDay,
  updateFlightField,
  HajjUploadFlightType,
} from "@/redux/feature/hajj-upload/hajjUploadFlightsSlice";

import { cn } from "@/lib/utils";
import {
  clearFlightError,
  setFlightError,
} from "@/redux/feature/hajj-upload/hajjUploadFlightErrorsSlice";
...
const HajjFlightForm: React.FC = () => {
  ...
  const [showFlightToDropdown, setShowFlightToDropdown] = useState<
    number | null
  >(null);
  const [showFlightTypeDropdown, setShowFlightTypeDropdown] = useState<
    number | null
  >(null);
  ...
  const flightTypeRefs = useRef<(HTMLDivElement | null)[]>([]);
  ...
  useEffect(() => {
    ...
    flightTypeRefs.current = flightTypeRefs.current.slice(0, flights.length);
  }, [flights]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        showFlightTypeDropdown !== null &&
        flightTypeRefs.current[showFlightTypeDropdown] &&
        !flightTypeRefs.current[showFlightTypeDropdown]?.contains(
          event.target as Node
        )
      ) {
        setShowFlightTypeDropdown(null);
      }
    };
  ...
  const validateFlightField = (
    ...
    if (field === "transitType") {
      if (!value.trim()) return "Transit type is required.";
    }
    if (field === "airlinesName") {
      if (!value.trim()) return "Airlines name is required.";
    }
  ...
  const handleFlightTypeSelect = (index: number, type: string) => {
    handleInputChange(index, "flightType", type);
    setShowFlightTypeDropdown(null);
  };
  ...
  return (
    <div className="p-4">
      <h2 className="text-lg font-medium mb-4">Flights</h2>
        ...
        {flights.map((flight, index) => (
          <div key={index} className="space-y-5 pb-4">
            ...
            <div>
              ...
              >
                <input
                  id={`baseFare-${index}`}
                  type="checkbox"
                  checked={flight.includedWithBasePrice}
                  onChange={(e) =>
                    handleInputChange(
                      index,
            ...
            <div className="flex gap-3">
              ...
              <div
                ref={(el) => {
                  flightTypeRefs.current[index] = el;
                }}
                className="relative w-full"
              >
                <Label
                ...
                <div
                  ...
                  onClick={() => setShowFlightTypeDropdown(index)}
                >
                  {flight.flightType || "Select flight type"}
                </div>

                <div
                  className={cn(
                    "absolute z-10 bg-white border w-full rounded-md shadow-md pb-1 transition-all duration-300 transform",
                    showFlightTypeDropdown === index
                      ? "opacity-100 translate-y-3"
                      : "opacity-0 translate-y-0 pointer-events-none"
                  )}
                >
                  {flightTypes.map((type) => (
                    <div
                      key={type}
                      className="px-2.5 py-1 cursor-pointer hover:text-white hover:bg-blue-500 "
                      onClick={() => handleFlightTypeSelect(index, type)}
                    >
                      {type}
                    </div>
                  ))}
            ...
            <div className="flex gap-3">
              ...
              <div className="relative w-full">
                ...
                <Input
                  ...
                  disabled={flight.includedWithBasePrice}
                />
                {flightErrors[index]?.pricePerPax && (
                  <Paragraph className="mt-1.5 text-red-500 text-xs">
                    {flightErrors[index]?.pricePerPax}
            ...
            {index !== 0 && (
              <Button
                type="button"
                onClick={() => dispatch(removeFlight(index))}
